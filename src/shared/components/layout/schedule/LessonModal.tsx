...
import { Student, CreateLessonDto, createLesson, getTutorStudents, Lesson, deleteLesson, updateLesson } from '@api/schedule';
...
const generateTimeOptions = (startHour: number, endHour: number, stepMinutes: number): OptionType[] => {
    ...
    return uniqueOptions.filter(time => {
...
export const LessonModal: React.FC<LessonModalProps> = ({ isOpen, onClose, isReadOnly = false, initialDate, onCreateSuccess, lesson, onDeleteSuccess, onUpdateSuccess }) => {
    ...
    const findKeyByValue = (map: Record<string, string>, value: string | undefined): string | undefined => {
        if (!value) return undefined;
        return Object.keys(map).find(key => map[key] === value);
    };

    const initialLessonType = LESSON_TYPE_OPTIONS[0]?.value || LessonType.Mathematics;
    const initialLessonCategory = LESSON_CONFIG_MAP[initialLessonType]?.[0] || LessonCategory.School;

    const [selectedStudentId, setSelectedStudentId] = useState<string>('');
    const [lessonType, setLessonType] = useState<LessonType>(initialLessonType);
    const [lessonCategory, setLessonCategory] = useState<LessonCategory>(initialLessonCategory);
    const [note, setNote] = useState<string>('');
    const [duration, setDuration] = useState<number>(Number(DURATION_OPTIONS[0]?.value) || 60);
    ...
    const studentOptions: OptionType[] = useMemo(() => (localStudents || []).map(s => ({
        value: s.id,
        label: `${(s as any).name || s.firstName || ''} ${s.lastName || ''}`
    })), [localStudents]);


    const initialStartTime = useMemo(() => getInitialTime(lesson, initialDate, TIME_OPTIONS), [lesson, initialDate]);
    ...
    const availableTimeOptions = useMemo(() => {
        ...
        if (isEditMode && lesson) {
            ...
            if (isTodayCutoff) {
                ...
                if (nextValidMinute >= 60) {
                    ...
                    nextValidTimeInMinutes = timeToMinutes(`${String(nextValidHour).padStart(2, '0')}:${String(minute).padStart(2, '0')}`);
                } else {
                    nextValidTimeInMinutes = timeToMinutes(`${String(nextValidHour).padStart(2, '0')}:${String(nextValidMinute).padStart(2, '0')}`);
                }
            }
        } else {
            return TIME_OPTIONS;
        ...
        if (isEditMode && lesson) {
            ...
            if (!filteredOptions.find(opt => opt.value === currentLessonStartTime)) {
                filteredOptions.push({ value: currentLessonStartTime, label: currentLessonStartTime });
                filteredOptions.sort((a, b) => timeToMinutes(a.value) - timeToMinutes(b.value));
            }
        }

        return filteredOptions;

    }, [isEditMode, lesson, initialDate, TIME_OPTIONS, getInitialTime]);
    ...
    const fetchStudents = async (): Promise<Student[]> => {
        ...
        try {
            ...
            const errorMessage = serverMessage || "Не удалось загрузить список студентов. Попробуйте обновить страницу.";
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (lesson) {
                ...
                const initialTypeKey = findKeyByValue(LESSON_TYPE_LABELS, lesson.lessonType) as LessonType || initialLessonType as LessonType;
                const initialCategoryKey = findKeyByValue(LESSON_CATEGORY_LABELS, lesson.lessonCategory) as LessonCategory || initialLessonCategory as LessonCategory;
                ...
                const lessonDuration = (localEnd.getTime() - localStart.getTime()) / (1000 * 60);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const baseDate = isCreateMode && initialDate ? initialDate : new Date(lesson!.startLessonDate);
        ...
        try {
            ...
            if (isCreateMode) {
                ...
                if (!lesson?.id || !onUpdateSuccess) throw new Error("ID занятия для обновления не найден.");
            ...
            const message = serverMessage || `Не удалось ${isCreateMode ? 'создать' : 'обновить'} занятие. Проверьте данные.`;
            setError(message);
        } finally {
            setIsLoading(false);
        }
    ...
    const dateToDisplay = isCreateMode && initialDate ? initialDate : (lesson ? new Date(lesson.startLessonDate) : null);
    ...
    if (!isOpen || (isCreateMode && availableTimeOptions.length === 0)) return null;
        ...
        showConfirm(
            ...
            async () => {
                ...
                try {
                    ...
                    const message = (err as any).response?.data?.message || 'Не удалось удалить занятие.';
                    setError(message);
    ...
    const handleLessonCategoryChange: SingleSelectChange = (newValue, _actionMeta) =>
        handleSelectChange(newValue, _actionMeta, setLessonCategory);


    const selectedStudentOption = studentOptions.find(o => o.value === selectedStudentId) || null;
    const selectedStartTimeOption = availableTimeOptions.find(o => o.value === startTime) || null;
    const selectedDurationOption = DURATION_OPTIONS.find(o => Number(o.value) === duration) || null;
    const selectedLessonTypeOption = LESSON_TYPE_OPTIONS.find((o: OptionType) => o.value === lessonType) || null;
    const selectedLessonCategoryOption = availableCategoryOptions.find(o => o.value === lessonCategory) || null;
        ...
        <div className={styles.modalOverlay} onClick={onClose}>
            <div className={styles.modalContent} onClick={e => e.stopPropagation()}>
                ...
                {availableTimeOptions.length === 0 && (
                    <p className={styles.errorText}>В выбранном часовом интервале нет доступного времени для начала занятия.</p>
                )}

                <form onSubmit={canPerformActions ? handleSubmit : (e) => e.preventDefault()}>
                    <div className={styles.formSection}>
                        <label>Дата занятия:</label>
                        <input type="text" value={displayDate || ''} readOnly className={styles.readOnlyInput} />
                    </div>
                    ...
                    <div className={styles.timeGroup}>
                        <div className={styles.formGroup}>
                            ...
                            <Select
                                ...
                                isDisabled={isLoading || availableTimeOptions.length === 0 || isReadOnly}
                                placeholder="--:--"
                    ...
                    <div className={styles.formGroup}>
                        ...
                        <Select
                            ...
                            isDisabled={isLoading || isStudentsLoading || (localStudents?.length || 0) === 0 || isReadOnly}
                            required
                            menuPortalTarget={document.body}
                            menuPosition="fixed"
                        />
                        {((localStudents?.length || 0) === 0 && !isStudentsLoading && localStudents !== null) && canPerformActions &&
                            <p className={styles.warningText}>Нет доступных студентов.</p>
                        }
                        {isStudentsLoading && <p className={styles.warningText}>Загрузка студентов...</p>}
                    </div>
                    ...
                    {canPerformActions && (
                        <div className={styles.buttonGroup}>
                            ...
                            <button
                                type="submit"
                                disabled={isLoading || !selectedStudentId || availableTimeOptions.length === 0}
                                className={styles.primaryButton}
                            >
                                {isLoading ? 'Сохранение...' : (isEditMode ? 'Сохранить изменения' : 'Создать занятие')}
                            </button>
                        </div>
                    )}
